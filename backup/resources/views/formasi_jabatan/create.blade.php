...
<div class="container">
    ...
    <form action="{{ route('admin.formasi-jabatan.store') }}" method="POST">
        @csrf

        <div class="mb-3">
            <label for="nama_formasi" class="form-label">Nama Formasi</label>
            <input type="text" name="nama_formasi" class="form-control" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="unit_kerja_id" class="form-control select2" required>
                ...
                @foreach ($unitKerja as $unit)
                    <option value="{{ $unit->id }}">{{ $unit->nama_rumahsakit }} - {{ $unit->kota_kabupaten }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" name="tahun_formasi" class="form-control" required>
        </div>
