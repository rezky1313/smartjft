  ...
  <div class="col-12">
      ...
      <div class="card-body py-3">
        @php
          $steps = [
            ...
            ['key' => 'diajukan_admin_unit',      'label' => 'Diajukan\nAdmin Unit'],
            ['key' => 'diverifikasi_admin_unit',  'label' => 'Diverifikasi\nAdmin Unit'],
            ['key' => 'diajukan_pusbin',          'label' => 'Diajukan\nPusbin'],
            ['key' => 'diverifikasi_pusbin',      'label' => 'Diverifikasi\nPusbin'],
            ['key' => 'selesai',                  'label' => 'Selesai'],
          ];
          $statusOrder = array_column($steps, 'key');
          $curIdx      = array_search($pendaftaran->status, $statusOrder);
        ...
        <div class="d-flex align-items-center justify-content-between">
          ...
          <div class="text-center flex-fill" style="font-size:0.78rem;">
            @php
              $isDone    = ($curIdx !== false && $sIdx < $curIdx) || ($pendaftaran->status === $step['key']);
              $isCurrent = $pendaftaran->status === $step['key'];
            @endphp
            <div class="mx-auto rounded-circle d-flex align-items-center justify-content-center"
                 style="width:36px;height:36px;
                   background: {{ $isCurrent ? '#007bff' : ($isDone ? '#28a745' : '#dee2e6') }};
            ...
            <div class="mt-1" style="line-height:1.2; white-space:pre-line; color:{{ $isCurrent ? '#007bff' : ($isDone ? '#28a745' : '#aaa') }}; font-weight:{{ $isCurrent ? '700' : '400' }};">
              {{ $step['label'] }}
            </div>
          </div>
          @if (!$loop->last)
          <div class="flex-fill" style="height:2px; background:#dee2e6; max-width:40px; margin-bottom:18px;"></div>
          @endif
          @endforeach
        </div>
        @endif
          ...
          <table class="table table-bordered table-sm mb-0">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><small>{{ $p->jabatan_tujuan_nama ?? ($p->jabatanTujuan?->nama_formasi ?? '-') }}</small></td>
                <td><small>{{ $p->jenjang_tujuan ?? ($p->jabatanTujuan?->jenjang?->nama_jenjang ?? '-') }}</small></td>
                <td>
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        {{-- Diajukan Admin Unit: Admin bisa Verifikasi & Tolak --}}
        ...
        <form action="{{ route('ujikom.permohonan.verifikasi.berkas', $pendaftaran->id) }}" method="POST" class="mb-3">
          ...
          <p class="mb-1 font-weight-bold small">{{ $p->pegawai?->nama_lengkap ?? '-' }}</p>
          @foreach ($berkasPeserta as $bIdx => $b)
          <div class="row mb-1 align-items-center ml-2">
            <div class="col-md-4 small">{{ $b->nama_berkas }}</div>
            <input type="hidden" name="berkas[{{ $loop->index }}][id]" value="{{ $b->id }}">
            <div class="col-md-3">
              <select name="berkas[{{ $loop->index }}][status_verifikasi]" class="form-control form-control-sm">
                <option value="belum"    {{ $b->status_verifikasi === 'belum'    ? 'selected' : '' }}>Belum</option>
            ...
            <div class="col-md-4">
              <input type="text" name="berkas[{{ $loop->index }}][catatan]" class="form-control form-control-sm"
          ...
          <button type="submit" class="btn btn-sm btn-info mt-2">
            <i class="fas fa-save mr-1"></i> Simpan Verifikasi Berkas
          </button>
        </form>
        <hr>
        @endif
        ...
        <form action="{{ route('ujikom.permohonan.verifikasi.admin', $pendaftaran->id) }}" method="POST" class="d-inline mr-2"
              onsubmit="return confirm('Verifikasi permohonan ini?')">
          @csrf
          <input type="hidden" name="catatan_admin_unit" value="Diverifikasi oleh Admin Unit.">
          <button type="submit" class="btn btn-success"><i class="fas fa-check mr-1"></i> Verifikasi</button>
        </form>
        ...
        {{-- Diverifikasi Admin Unit: Admin ajukan ke Pusbin --}}
        @if ($pendaftaran->status === 'diverifikasi_admin_unit')
        @role('admin|super_admin')
        <form action="{{ route('ujikom.permohonan.ajukan.pusbin', $pendaftaran->id) }}" method="POST" class="d-inline"
              onsubmit="return confirm('Ajukan ke Pusbin?')">
          @csrf
          <button type="submit" class="btn btn-primary"><i class="fas fa-paper-plane mr-1"></i> Ajukan ke Pusbin</button>
        </form>
        @endrole
        @endif

        {{-- Diajukan Pusbin: Admin verifikasi pusbin atau tolak --}}
        ...
        <form action="{{ route('ujikom.permohonan.verifikasi.pusbin', $pendaftaran->id) }}" method="POST" class="d-inline mr-2"
              onsubmit="return confirm('Verifikasi dan selesaikan permohonan ini?')">
          @csrf
          <button type="submit" class="btn btn-success"><i class="fas fa-check-double mr-1"></i> Verifikasi Pusbin & Selesaikan</button>
        </form>
        <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#modalTolakPusbin">
          <i class="fas fa-times mr-1"></i> Tolak Pusbin
        </button>
        @endrole
        @endif
