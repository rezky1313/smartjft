...
@section('title','Uji Kompetensi - Tambah')
@section('isi')
...
<div class="container mt-3">
  ...
  <h4 class="mb-3">Tambah Uji Kompetensi</h4>

  <form action="{{ route('user.uji.store') }}" method="POST">
    @csrf
    <div class="row g-3">
      ...
      <div class="col-md-12">
        ...
        <input type="hidden" name="sdm_id" id="sdm_id" value="{{ old('sdm_id') }}">
      ...
      <div class="col-md-4">
        ...
        <input type="text" id="f_nip" class="form-control" readonly>
      </div>
      <div class="col-md-4">
        <label class="form-label">Jenjang Jabatan</label>
        <input type="text" id="f_jenjang" class="form-control" readonly>
      </div>
      <div class="col-md-4">
        <label class="form-label">Nama Unit Kerja</label>
        <input type="text" id="f_unit" class="form-control" readonly>
      </div>
      <div class="col-md-4">
        <label class="form-label">Kab/Kota</label>
        <input type="text" id="f_kabkota" class="form-control" readonly>
      </div>
      <div class="col-md-4">
        <label class="form-label">Instansi</label>
        <input type="text" id="f_instansi" class="form-control" readonly>
      </div>
      ...
      <div class="col-md-4">
        ...
        <select name="kompetensi" class="form-select" required>
          ...
          @foreach(($opsiKompetensi ?? ['PT1','PT2','PT3','PT4','PT5','Perpanjangan']) as $opt)
            <option value="{{ $opt }}" @selected(old('kompetensi')===$opt)>{{ $opt }}</option>
          @endforeach
        </select>
      ...
      <div class="col-md-4">
        ...
        <input type="number" step="0.01" name="nilai" value="{{ old('nilai') }}" class="form-control">
        @error('nilai') <small class="text-danger">{{ $message }}</small> @enderror
      </div>

      <div class="col-md-4">
        <label class="form-label">Tanggal Uji Kompetensi</label>
        <input type="date" name="tanggal_uji" value="{{ old('tanggal_uji') }}" class="form-control">
        @error('tanggal_uji') <small class="text-danger">{{ $message }}</small> @enderror
      </div>

      <div class="col-md-6">
        <label class="form-label">Nomor Sertifikat</label>
        <input type="text" name="nomor_sertifikat" value="{{ old('nomor_sertifikat') }}" class="form-control">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  $('#sdmSelect').select2({
    ...
    allowClear: true,
    width: '100%',
    minimumInputLength: 0,                // biar tanpa ngetik pun bisa muncul
    ajax: {
      url: ajaxUrl,
      delay: 250,
      dataType: 'json',
  ...
  // Saat dropdown dibuka, trigger pencarian kosong supaya 20 data pertama tampil
  $('#sdmSelect').on('select2:open', function() {
    const $search = $('.select2-container--open .select2-search__field');
    $search.val('').trigger('input');
  });

  // Isi field setelah pilih SDM
  ...
  // Bersihkan field saat clear
  $('#sdmSelect').on('select2:clear', function () {
    $('#sdm_id').val('');
    ['f_nama','f_nip','f_jenjang','f_unit','f_kabkota','f_instansi'].forEach(id => {
...
{{-- <script>
document.addEventListener('DOMContentLoaded', function () {
  // Pastikan Select2 tersedia; jika tidak, fallback jadi select biasa.
  if (window.$ && $.fn.select2) {
    $('#sdmSelect').select2({
      placeholder: 'Ketik nama / NIP / unit / jenjang…',
      allowClear: true,
    ...
    // Jika old('sdm_id') ada (validasi gagal), bisa load manual detail via AJAX (opsional)
    @if(old('sdm_id'))
    $.getJSON('{{ route("user.uji.sdm-mini", old("sdm_id")) }}', function(d){
      if (d && d.id) {
